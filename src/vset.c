...
i32 main(i32 argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "hrv:d:c:m:G:a:u:f:o:w:M:t:", long_options, &option_index)) != -1) {
        switch(opt) {
            ...
            case 'c':
                if (optarg) {
                    i32 parsed = atoi(optarg);
                    if (parsed < 0) {
                        fprintf(stderr, ERR "-c cannot be negative\n");
                        return EXIT_FAILURE;
                    }
                    if (!validate_core_count_arg((u32)parsed))
                        return EXIT_FAILURE;
                    if (parsed % 2 != 0) {
                        fprintf(stderr, ERR "-c requires an even number of cores\n");
                        return EXIT_FAILURE;
                    }
                    g_config.num_threads = (u32)parsed;
                } else {
            ...
            case 'u':
                granular = true;
                {
                    ...
                    g_config.num_l2_sets = (u32)parsed;
                }
                u_provided = true;
                break;
            case 'f':
                granular = true;
                {
                    ...
                    g_config.evsets_per_l2 = (u32)parsed;
                }
                f_provided = true;
                break;
            case 'o':
                granular = true;
                {
                    ...
                    if ((u32)parsed > NUM_OFFSETS) {
                        printf(WRN "requested offsets (%u) > max available (%lu). setting to %lu\n",
                               (u32)parsed, NUM_OFFSETS, NUM_OFFSETS);
                        parsed = NUM_OFFSETS;
                    }
                    g_config.num_offsets = (u32)parsed;
                }
                o_provided = true;
                break;
            case 'G':
    ...
    if (heatmap_time_step_us == 0) {
        fprintf(stderr, ERR "-t/--time-step must be greater than 0\n");
        return EXIT_FAILURE;
    }
    ...
    bool lcas_single = lcas_mode &&
                       g_config.num_l2_sets == 1 &&
                       g_config.evsets_per_l2 == 1 &&
                       g_config.num_offsets == 1;
    bool graph_needs_hi_cgroup = graph_mode &&
                                 graph_type != GRAPH_L2COLOR_DIST &&
                                 !(graph_type == GRAPH_EVRATE_TIME && lcas_single);
    bool needs_hi_cgroup = live_mode || perf_mode || freq_mode ||
                           (lcas_mode && !lcas_single) ||
                           graph_needs_hi_cgroup;
    if (needs_hi_cgroup && check_cgroup_hi_ready() == -1)
        return EXIT_FAILURE;

    init_cache_info();

    if (g_config.num_l2_sets > g_n_uncertain_l2_sets) {
        printf(WRN "requested L2 sets (%u) > max uncertain sets (%u). setting to %u\n",
               g_config.num_l2_sets, g_n_uncertain_l2_sets, g_n_uncertain_l2_sets);
        g_config.num_l2_sets = g_n_uncertain_l2_sets;
    }

    u64 max_f = cache_uncertainty(&l3_info) / cache_uncertainty(&l2_info);
    if (max_f == 0) {
        fprintf(stderr, ERR "failed to calculate max -f for this topology\n");
        return EXIT_FAILURE;
    }
    if (g_config.evsets_per_l2 > max_f) {
        fprintf(stderr, ERR "Requested -f %u but only max f is %lu\n",
                g_config.evsets_per_l2, max_f);
        return EXIT_FAILURE;
    }
