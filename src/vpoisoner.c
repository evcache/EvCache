...
i32 main(i32 argc, char *argv[])
{
    ...
    if (g_config.num_threads == 1) {
        fprintf(stderr, ERR "-c requires at least 2 cores for paired eviction-set construction\n");
        return EXIT_FAILURE;
    }
    if (g_config.num_threads > 1 && g_config.num_threads % 2 != 0) {
        printf(WRN "-c uses core pairs; using %u cores\n", g_config.num_threads - 1);
    ...
    } else if (g_config.num_l2_sets > g_n_uncertain_l2_sets) {
