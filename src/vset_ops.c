...
#define HI_CGROUP_PROCS "/sys/fs/cgroup/hi_prgroup/cgroup.procs"
...
i32 check_cgroup_hi_ready(void)
{
    if (access(HI_CGROUP_PROCS, W_OK) == -1) {
        fprintf(stderr, ERR "High-priority cgroup is not ready (%s: %s).\n"
                            "1) Running as root?\n"
                            "2) Ran scripts/setup_vset.sh ?\n",
                HI_CGROUP_PROCS, strerror(errno));
        return -1;
    }
