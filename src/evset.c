                      ...
                      i32 thread_id, u32 filter_offset)
{
    ...
    if (thread_id >= 0)
        printf("├─ Thread %3d: ", thread_id);
    else
        printf("%s", INFO);
    printf("Filtered %lu candidate lines to %*ld | %.3fms",
           n_cands_init, w, n_cands_filtered, (end - start) / 1e3);
...
EvTestRes test_eviction_detailed(u8 *target, u8 **cands, u64 cnt,
                                 EvBuildConf *tconf)
{
    ...
    u32 unsure_retry = tconf->unsure_retry ? tconf->unsure_retry : 1;
    ...
    for (u32 r = 0; r < unsure_retry; r++) {
        otc = 0;
        u32 valid_trials = 0;
        u32 attempts = 0;
        u32 max_attempts = trials * 4;

        while (valid_trials < trials && attempts < max_attempts) {
            ...
            if (aux_before != aux_after || lat > g_lats.interrupt_thresh)
                continue;

            valid_trials++;
            if (lat >= tconf->lat_thresh)
                otc++;
...
{
    return test_eviction_detailed(target, cands, cnt, tconf) > 0 ? OK : FAIL;
}
                          ...
                          EvSet* filter_ev, EvBuildConf* conf)
{
    ...
    for (u64 s = 0; s < total_cands; s += batch_sz) {
        ...
        for (u32 t = 0; t < conf->trials; t++) {
            access_array(&addrs[s], cur_batch_sz);
            _lfence();
...
*/
static bool addr_in_list(u8 *addr, u8 **addrs, u64 count)
{
...
static bool pick_l2_target(EvCands *cands, u8 *current_target,
                           u8 **prev_addrs, u64 n_prev_addrs,
                           EvBuildConf *conf, u8 **target)
{
    if (!cands || !target)
        return false;

    for (u64 j = 0; j < cands->count; j++) {
        u8 *candidate = cands->addrs[j];
        if (candidate == current_target || addr_in_list(candidate, prev_addrs, n_prev_addrs))
            continue;

        if (n_prev_addrs > 0 &&
            test_eviction_detailed(candidate, prev_addrs, n_prev_addrs, conf) != EV_NEG)
            continue;
...
static bool validate_l2_evsets(EvSet **evsets, u32 count, EvBuildConf *conf)
{
    if (!evsets || count == 0)
        return false;

    for (u32 i = 0; i < count; i++) {
        if (!evsets[i] || !evsets[i]->target_addr ||
            verify_evset(evsets[i], evsets[i]->target_addr) == FAIL) {
            if (verbose > 1)
                printf(V2 WRN "L2 evset %u failed group self-test\n", i);
            return false;
        }
    }
    ...
    for (u32 i = 0; i < count - 1; i++) {
        for (u32 j = i + 1; j < count; j++) {
            EvTestRes res = test_eviction_detailed(evsets[j]->target_addr,
                                                   evsets[i]->addrs,
                                                   evsets[i]->size,
                                                   conf);
            if (res != EV_NEG) {
                if (verbose > 1)
                    printf(V2 WRN "L2 evsets %u and %u are not clearly independent\n", i, j);
                return false;
            }
        }
    }
                             ...
                             u8** all_addrs, u64 n_all_addrs) 
{
    ...
    bool local_cands = false;

    if (!cands) {
        cands = evcands_new(&l2_info, conf, NULL);
        if (!cands) {
            fprintf(stderr, ERR "failed to allocate candidates for L2 set %u\n", set_idx);
            return NULL;
        }

        if (evcands_populate(0, cands, conf, -1, 0)) {
            fprintf(stderr, ERR "Failed to populate candidates for L2 set %u\n", set_idx);
            evcands_free(cands);
            return NULL;
        }
    ...
    if (!target && !pick_l2_target(cands, NULL, all_addrs, n_all_addrs, conf, &target)) {
        fprintf(stderr, ERR "failed to select target for L2 set %u\n", set_idx);
        if (local_cands)
            evcands_free(cands);
        return NULL;
    }
    ...
    while (n_ret > 0 && !success) {
        ...
        if (build_evset_zhao(curr_target, evset)) {
            ...
            if (!pick_l2_target(cands, curr_target, all_addrs, n_all_addrs,
                                conf, &curr_target)) {
...
EvSet ***build_l2_evset(u32 num_sets)
{
    ...
    while (whole_ret <= max_whole_ret) {
        ...
        EvCands* cands = evcands_new(&l2_info, &def_l2_build_conf, NULL);
        if (!cands) {
        ...
        if (evcands_populate(0, cands, &def_l2_build_conf, -1, 0)) {
            fprintf(stderr, ERR "Failed to populate eviction candidates\n");
            evcands_free(cands);
            free(l2_evset_complex);
            return NULL;
        }
        ...
        u8* target = NULL;
        if (!pick_l2_target(cands, NULL, NULL, 0, &def_l2_build_conf, &target)) {
            fprintf(stderr, ERR "Failed to find initial target for L2 evset construction\n");
            evcands_free(cands);
            free(l2_evset_complex[0]);
            free(l2_evset_complex);
            return NULL;
        }
        ...
        for (u32 i = 0; i < uncertain_sets_to_build; i++) {
            ...
            if (i > 0) {
                if (!pick_l2_target(cands, target, built_evset_addrs,
                                    n_built_evset_addrs,
                                    &def_l2_build_conf, &target)) {
            ...
            if (l2_evset_complex[0][i]) {
                ...
                for (i32 verify_retry = 0; verify_retry < 3; verify_retry++) {
                    if (verify_evset(l2_evset_complex[0][i],
                                     l2_evset_complex[0][i]->target_addr) == OK) {
                ...
                if (verification_failed) {
                    ...
                    if (debug > 2) {
                        printf(D3 "Target that failed eviction: VA %p -> HPA 0x%lx\n",
                            l2_evset_complex[0][i]->target_addr,
                            va_to_hpa(l2_evset_complex[0][i]->target_addr));
        ...
        bool group_valid = success_count == uncertain_sets_to_build &&
                           validate_l2_evsets(l2_evset_complex[0],
                                              uncertain_sets_to_build,
                                              &def_l2_build_conf);
        if (success_count == uncertain_sets_to_build && !group_valid && verbose)
            printf(WRN "Built all uncertain sets, but group validation failed\n");
...
bool final_l3_evset(EvSet* l3ev)
{
    ...
    for (u32 r = 0; r < ret; r++) {
        ...
        if (!l3ev->cands || l3ev->cands->count == 0) {
            if (verbose > 1)
                printf("     " V2 "No L3 candidates left for retry\n");
            break;
        }
...
EvSet** build_single_l3_evset(void)
{
    ...
    EvCands* l3_cands = evcands_new(&l3_info, &def_l3_build_conf, NULL);
    if (!l3_cands) {
    ...
    if (evcands_populate(0, l3_cands, &def_l3_build_conf, -1, 0)) {
        fprintf(stderr, ERR "failed to populate L3 candidates\n");
        evcands_free(l3_cands);
        return NULL;
    }

    u64 n_filtered = l3_cands->count;
    if (n_filtered <= 1) {
    ...
    u8* target = l3_cands->addrs[n_filtered - 1];
                        ...
                        u64 n_lower_evsets, u32 max_evsets)
{
    ...
    EvSet **evsets = NULL;
    u64 n_possible = cache_uncertainty(cache), lower_skipped = 0;
    if (conf->filter_ev) {
        CacheInfo *lower = conf->filter_ev->target_cache;
        n_possible /= cache_uncertainty(lower);
    }
    u64 result_cap = max_evsets > 0 ? max_evsets : n_possible;
    u64 build_limit = _min(n_possible, result_cap);
    *ev_cnt = result_cap;
    ...
    evsets = _calloc(result_cap, sizeof(EvSet*));
    ...
    for (u64 i = 0; i < build_limit && cands->count > 0; i++) {
        EvSet *evset = NULL;
        if (n_lower_evsets) {
            u64 start = time_us();
            conf->lower_ev = NULL;
        ...
        if (!build_error && evset->size != 0) {
            ...
            evsets[i] = evset;
            if (!addrs) {
                addrs = _calloc(evset->ev_cap * build_limit, sizeof(*addrs));
                if (!addrs)
                    goto err;
            }
    ...
    l2_ev_fail:
        if (i == build_limit - 1 || cands->count == 0) break;
