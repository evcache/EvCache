                                    ...
                                    EvSet ***pre_l2evsets)
{
    ...
    if (g_config.num_threads == 1) {
        fprintf(stderr, ERR "Need at least 2 cores for paired eviction-set construction\n");
        return NULL;
    }

    u32 usable_cores = g_config.num_threads == 0 ? (u32)n_cores : g_config.num_threads;
    if (usable_cores > (u32)n_cores) {
        u32 capped_cores = (u32)n_cores;
        if (capped_cores % 2 != 0)
            capped_cores--;
        printf(WRN "requested cores (%u) > available cores (%d). setting to %u\n",
               usable_cores, n_cores, capped_cores);
        usable_cores = capped_cores;
    }
    if (usable_cores % 2 != 0)
        usable_cores--;

    u32 n_pairs = usable_cores / 2;
    ...
    } else if (n_l2_sets > n_unc_l2_sets) {
    ...
    printf(INFO "Requested: %u eviction set(s) per L2 uncertain set (%u sets) across %u offsets\n",
...
EvSet**** build_l3_evsets_para(u32 n_offset)
{
    ...
    if (n_offset > NUM_OFFSETS) {
        printf(WRN "requested offsets (%u) > max available (%lu). setting to %lu\n",
               n_offset, NUM_OFFSETS, NUM_OFFSETS);
