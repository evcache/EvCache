...
static bool parse_cpu_dir_name(const char *name, i32 *cpu_id)
{
    char *end = NULL;
    long parsed;
    ...
    errno = 0;
    parsed = strtol(name + 3, &end, 10);
    if (errno != 0 || end == name + 3 || *end != '\0' || parsed < 0 || parsed > INT_MAX)
        return false;
...
static bool read_i32_file(const char *path, i32 *value)
{
    ...
    if (fscanf(file, "%ld", &parsed) != 1 || parsed < INT_MIN || parsed > INT_MAX) {
        fclose(file);
        return false;
    }
...
static bool cpu_is_online(i32 cpu_id)
{
    char path[128];
    i32 online = 1;

    snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%d/online", cpu_id);
    if (!read_i32_file(path, &online))
        return true;

    return online != 0;
}

static bool physical_core_seen(const physical_core_id_t *cores, u32 n_cores,
                               i32 package_id, i32 core_id)
{
...
i32 n_physical_cores(void)
{
    DIR *cpu_dir = opendir("/sys/devices/system/cpu");
    struct dirent *entry;
    physical_core_id_t *cores;
    u32 n_cores = 0;
    u32 cap;

    if (!cpu_dir)
        return n_system_cores();

    cap = (u32)n_system_cores();
    if (cap == 0)
        cap = 1;

    cores = calloc(cap, sizeof(*cores));
    if (!cores) {
        closedir(cpu_dir);
        return n_system_cores();
    }
    ...
    while ((entry = readdir(cpu_dir)) != NULL) {
        ...
        if (!parse_cpu_dir_name(entry->d_name, &cpu_id))
            continue;
        ...
        if (!read_i32_file(path, &package_id))
            continue;

        snprintf(path, sizeof(path), "/sys/devices/system/cpu/cpu%d/topology/core_id", cpu_id);
        if (!read_i32_file(path, &core_id))
            continue;

        if (physical_core_seen(cores, n_cores, package_id, core_id))
            continue;

        if (n_cores == cap) {
            u32 new_cap = cap * 2;
            physical_core_id_t *new_cores = realloc(cores, new_cap * sizeof(*cores));
            if (!new_cores)
                break;
        ...
        cores[n_cores++] = (physical_core_id_t){ .package_id = package_id, .core_id = core_id };
    }
...
bool validate_core_count_arg(u32 requested)
{
    i32 max_cores;
    ...
    max_cores = n_physical_cores();
    if (max_cores <= 0)
        max_cores = n_system_cores();

    if (requested > (u32)max_cores) {
        fprintf(stderr, ERR "Requested -c %u but only max c is %d\n", requested, max_cores);
        return false;
    }
