...
const LCAS_MAP_PATH: &str = "/sys/fs/bpf/lcas_dom_order";
...
impl<'a> Scheduler<'a> {
    fn init(opts: &Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        if Path::new(LCAS_MAP_PATH).exists() {
            std::fs::remove_file(LCAS_MAP_PATH)?;
        }
        skel.maps.lcas_dom_order.pin(LCAS_MAP_PATH)?;
        let no_pref = (domains.nr_doms() as u32).to_ne_bytes();
        skel.maps.lcas_dom_order.update(
            &(0u32.to_ne_bytes()),
            &no_pref,
    ...
    fn migrate_tasks_to_domain(&mut self, dom: u32) -> Result<()> {
        if dom as usize >= self.dom_group.nr_doms() {
            return Ok(());
        }

        const MAX_TPTRS: usize = bpf_intf::consts_MAX_DOM_ACTIVE_TPTRS as usize;

        for (id, d) in self.dom_group.doms() {
            if *id == dom as usize {
                continue;
            }
            if let Some(domc) = d.ctx() {
                let active = &mut domc.active_tasks;
                let mut ridx = active.read_idx;
                let widx = active.write_idx;
                if widx - ridx > MAX_TPTRS as u64 {
                    ridx = widx - MAX_TPTRS as u64;
                }
                for idx in ridx..widx {
                    let ptr = active.tasks[(idx % MAX_TPTRS as u64) as usize];
                    if ptr.is_null() {
                        continue;
                    }
                    unsafe {
                        let taskc = &mut *ptr;
                        if (taskc.dom_mask & (1u64 << dom)) != 0 {
                            taskc.target_dom = dom;
                        }
                    }
                }
            }
    ...
    fn check_lcas_order(&mut self) -> Result<()> {
        let nr_doms = self.skel.maps.rodata_data.as_ref().unwrap().nr_doms;
        let mut order = Vec::new();
        for idx in 0..nr_doms {
            ...
            {
                order.push(u32::from_ne_bytes(v.as_slice().try_into().unwrap()));
            }
        }
        let display_order = if !order.is_empty() && order[0] < nr_doms {
            order.clone()
        } else {
            Vec::new()
        };
        if display_order != self.lcas_prev_order {
            println!("[LCAS] preferred socket: {:?}", display_order);
            if let Some(&dom) = display_order.get(0) {
                self.migrate_tasks_to_domain(dom)?;
            }
            self.lcas_prev_order = display_order;
        }
        Ok(())
    }
